...
type RsvpRow = {
  ...
  guest_names: string; dietary: string; guest_dietary?: string; transport: string; song: string; favorite_movie?: string; message: string; created_at: string;
};
type GiftRow = {
...
export default function AdminPage() {
  ...
  function downloadCsv() {
    ...
    const rows = rsvps.map((row) => [row.full_name, row.attendance === "yes" ? "Viene" : "No viene", row.guest_count, row.guest_names, row.dietary, row.guest_dietary ?? "", row.transport === "yes" ? "Sí" : "No", row.song, row.favorite_movie ?? "", row.email, row.created_at]);
  ...
  return (
    <main style={{ fontFamily: "Arial, sans-serif", padding: "48px 4vw 80px", background: "#10100f", color: "#f5f2eb", minHeight: "100vh" }}>
      ...
      <h2>Asistencia</h2>
      <div style={{ overflowX: "auto", background: "#191918", border: "1px solid #383838" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1380 }}><thead><tr>{["Nombre", "Respuesta", "Cantidad", "Acompañantes", "Comida titular", "Comida acompañantes", "Transporte", "Canción", "Película favorita", "Email", "Fecha", ""].map((h, index) => <th key={`${h}-${index}`} style={{ textAlign: "left", padding: 12, borderBottom: "1px solid #555", color: "#f5f2eb" }}>{h}</th>)}</tr></thead><tbody>{rsvps.map((row) => <tr key={row.id}><td style={cellStyle}>{row.full_name}</td><td style={cellStyle}>{row.attendance === "yes" ? "Viene" : "No viene"}</td><td style={cellStyle}>{row.guest_count}</td><td style={cellStyle}>{row.guest_names}</td><td style={cellStyle}>{row.dietary}</td><td style={cellStyle}>{row.guest_dietary ?? ""}</td><td style={cellStyle}>{row.transport === "yes" ? "Sí" : "No"}</td><td style={cellStyle}>{row.song}</td><td style={cellStyle}>{row.favorite_movie ?? ""}</td><td style={cellStyle}>{row.email}</td><td style={cellStyle}>{new Date(row.created_at).toLocaleDateString("es-AR")}</td><td style={cellStyle}><DeleteRecordButton label={`la confirmación de ${row.full_name}`} onDelete={() => deleteDoc(doc(firestore, "rsvps", row.id))} /></td></tr>)}</tbody></table></div>
