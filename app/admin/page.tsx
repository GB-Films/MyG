...
export default function AdminPage() {
  ...
  function downloadCsv() {
    const header = ["Nombre", "Respuesta", "Cantidad", "Acompañantes", "Comida", "Transporte", "Email", "Fecha"];
    const rows = rsvps.map((row) => [row.full_name, row.attendance === "yes" ? "Viene" : "No viene", row.guest_count, row.guest_names, row.dietary, row.transport === "yes" ? "Sí" : "No", row.email, row.created_at]);
